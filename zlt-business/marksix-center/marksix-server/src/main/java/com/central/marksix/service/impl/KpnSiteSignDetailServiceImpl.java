...
import com.central.marksix.entity.vo.KpnSiteUserSignHistoryVo;
import com.central.marksix.entity.vo.KpnSiteUserSignResultVo;
import com.central.marksix.mapper.KpnSiteSignDetailMapper;
...
public class KpnSiteSignDetailServiceImpl extends SuperServiceImpl<KpnSiteSignDetailMapper, KpnSiteSignDetail> implements IKpnSiteSignDetailService {
    ...
    public KpnSiteUserSignResultVo sign(SysUser sysUser, String date) {
        ...
        KpnSiteSignDetail yesterdaySign = this.lambdaQuery().select(KpnSiteSignDetail::getDays).eq(KpnSiteSignDetail::getUserId, sysUser.getId()).eq(KpnSiteSignDetail::getSignDate, yesterday).orderByDesc(KpnSiteSignDetail::getId).last(MarksixConstants.Sql.LIMIT_ONLY_ONE).one();
        ...
        //保存今日签到
        KpnSiteSignDetail todaySign = KpnSiteSignDetail.builder().siteId(sysUser.getSiteId()).siteCode(sysUser.getSiteCode()).siteName(sysUser.getSiteName()).userId(sysUser.getId()).username(sysUser.getUsername()).days(signDays).signMonth(StrUtil.subBefore(date, MarksixConstants.Symbol.ARRIVE, true)).signDate(date).build();
    ...
    public boolean checkHasSigned(Long userId, String date) {
        KpnSiteSignDetail signDetail = this.lambdaQuery()
                ...
                .eq(KpnSiteSignDetail::getSignMonth, StrUtil.subBefore(date, MarksixConstants.Symbol.ARRIVE, true))
    ...
    public Integer getUserSignDays(Long userId) {
        KpnSiteSignDetail signDetail = this.lambdaQuery().eq(KpnSiteSignDetail::getUserId, userId).orderByDesc(KpnSiteSignDetail::getSignDate).last(MarksixConstants.Sql.LIMIT_ONLY_ONE).one();
