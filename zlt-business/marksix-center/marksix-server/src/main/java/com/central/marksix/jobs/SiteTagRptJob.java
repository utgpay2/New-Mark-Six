...
//                    //按播放量高->低
//                    String redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TAG_MOVIEID_VV, sid, tagId);
//                    List<Long> movieIdsByVvDesc = movieTagService.getTagMovieIdsSortedByColumn(sid, tagId, MarksixConstants.Sql.COLUMN_VV);
...
//
//                    //按影片时长高->低
//                    redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TAG_MOVIEID_DURATION, sid, tagId);
//                    if (RedisRepository.length(redisKey) <= 0) {
//                        List<Long> movieIdsByDuration = movieTagService.getTagMovieIdsSortedByColumn(sid, tagId, MarksixConstants.Sql.COLUMN_DURATION);
...
//
//                    //按影片创建时间新->旧
//                    redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TAG_MOVIEID_LATEST, sid, tagId);
//                    if (RedisRepository.length(redisKey) <= 0) {
//                        List<Long> movieIdsByCreateTime = movieTagService.getTagMovieIdsSortedByColumn(sid, tagId, MarksixConstants.Sql.COLUMN_CREATE_TIME);
