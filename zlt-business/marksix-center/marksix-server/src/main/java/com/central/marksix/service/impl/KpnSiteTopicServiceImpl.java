...
import com.central.marksix.entity.vo.KpnSiteTopicVo;
import com.central.marksix.mapper.KpnSiteTopicMapper;
...
public class KpnSiteTopicServiceImpl extends SuperServiceImpl<KpnSiteTopicMapper, KpnSiteTopic> implements IKpnSiteTopicService {
    ...
    public List<Long> getTopicIdsBySiteId(Long sid) {
        String topicKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TOPIC_KEY, sid);
        ...
        if (CollectionUtil.isEmpty(siteTopics)) {
            ...
            if (CollectionUtil.isNotEmpty(siteTopics)) {
                RedisRepository.setExpire(topicKey, siteTopics, MarksixConstants.RedisKey.EXPIRE_TIME_30_DAYS);
            }
        }
