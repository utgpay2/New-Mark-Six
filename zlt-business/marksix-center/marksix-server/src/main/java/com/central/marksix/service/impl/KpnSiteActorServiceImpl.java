...
import com.central.marksix.entity.vo.KpnActorVo;
import com.central.marksix.mapper.KpnSiteActorMapper;
...
public class KpnSiteActorServiceImpl extends SuperServiceImpl<KpnSiteActorMapper, KpnSiteActor> implements IKpnSiteActorService {
    ...
    public KpnActorVo getKpnActorVo(Long sid, Long actorId) {
        ...
        if (StrUtil.isNotBlank(kpnActorVo.getAvatarUrl())) {
            kpnActorVo.setAvatarUrl(externalEndpoint + MarksixConstants.Symbol.FORWARD_SLASH + kpnActorVo.getAvatarUrl());
        }
        if (StrUtil.isNotBlank(kpnActorVo.getPortraitUrl())) {
            kpnActorVo.setPortraitUrl(externalEndpoint + MarksixConstants.Symbol.FORWARD_SLASH + kpnActorVo.getPortraitUrl());
        }
        //站点演员收藏量
    ...
    public Long getSiteActorFavorites(Long sid, Long actorId) {
        String siteActorFavoritesKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_ACTOR_FAVORITES_KEY, sid, actorId);
        ...
        if (ObjectUtil.isEmpty(siteActorFavoritesObj)) {
            ...
            try {
                ...
                if (ObjectUtil.isEmpty(siteActorFavoritesObj)) {
                    ...
                    RedisRepository.setExpire(siteActorFavoritesKey, favorites, MarksixConstants.RedisKey.EXPIRE_TIME_30_DAYS);
                }
            } catch (Exception e) {
