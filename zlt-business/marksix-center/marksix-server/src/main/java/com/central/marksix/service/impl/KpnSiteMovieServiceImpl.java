...
import com.central.marksix.entity.vo.KpnMovieVo;
import com.central.marksix.entity.vo.KpnSiteMovieBaseVo;
import com.central.marksix.entity.vo.KpnTagVo;
import com.central.marksix.enums.KpnMovieSortTypeEnum;
import com.central.marksix.enums.KpnSiteMovieSearchFromEnum;
import com.central.marksix.mapper.KpnSiteMovieMapper;
...
public class KpnSiteMovieServiceImpl extends SuperServiceImpl<KpnSiteMovieMapper, KpnSiteMovie> implements IKpnSiteMovieService {
    ...
    public List<KpnSiteMovieBaseVo> getSiteMovieByIds(Long sid, List<Long> movieIds, Boolean isDetail) {
        ...
        List<String> redisKeyVoList = movieIds.stream().map(movieId -> StrUtil.format(MarksixConstants.RedisKey.KPN_SITEID_MOVIEID_VO_KEY, sid, movieId)).collect(Collectors.toList());
        ...
        if (hasNullVoElem) {
            ...
            if (CollectionUtil.isNotEmpty(cachedKpnMovieVos)) {
                for (KpnMovieVo kpnMovieVo : cachedKpnMovieVos) {
                    RedisRepository.setExpire(StrUtil.format(MarksixConstants.RedisKey.KPN_SITEID_MOVIEID_VO_KEY, sid, kpnMovieVo.getId()), kpnMovieVo, MarksixConstants.RedisKey.EXPIRE_TIME_30_DAYS);
                }
            }
        }
    ...
    public KpnMovieVo getSiteMovieDetail(Long sid, Long movieId) {
        String movieRedisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITEID_MOVIEID_VO_KEY, sid, movieId);
    ...
    public KpnSiteMovie getSiteMovieVvFavorites(Long sid, Long movieId) {
        //先缓存
        String siteMovieVvKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_MOVIE_VV_KEY, sid, movieId);
        cacheSiteMovieVv(siteMovieVvKey, sid, movieId);
        long vv = RedisRepository.incr(siteMovieVvKey);

        String siteMovieFavoritesKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_MOVIE_FAVORITES_KEY, sid, movieId);
    ...
    public Long getSiteActorMovieNum(Long sid, Long actorId) {
        String redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_ACTOR_MOVIENUM_KEY, sid, actorId);
        ...
        if (ObjectUtil.isEmpty(siteActorMovieNum)) {
            ...
            if (ObjectUtil.isNotEmpty(siteActorMovieNum)) {
                RedisRepository.setExpire(redisKey, siteActorMovieNum, MarksixConstants.RedisKey.EXPIRE_TIME_30_DAYS);
            }
        }
...
//        siteMovieBaseVos.sort(KpnSiteMovieBaseVo::compareByVv);
    ...
    public PornPageResult<KpnSiteMovieBaseVo> searchDepot(Long sid, Integer from, Long fromId, String sortType, Integer sortOrder, MovieSearchConditionDto searchConditionDto, Integer currPage, Integer pageSize) {
        ...
        //标签
            ...
            //最热 默认
            String redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TAG_MOVIEID_VV, sid, fromId);
            //最新
            if (sortType.equalsIgnoreCase(KpnMovieSortTypeEnum.LATEST.getType())) {
                redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TAG_MOVIEID_LATEST, sid, fromId);
            }
            //时长
            else if (sortType.equalsIgnoreCase(KpnMovieSortTypeEnum.DURATION.getType())) {
                redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TAG_MOVIEID_DURATION, sid, fromId);
            }
        ...
        //主题 TOPIC
            ...
            //最热 默认
            String redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TOPIC_MOVIEID_VV, sid, fromId);
            //最新
            if (sortType.equalsIgnoreCase(KpnMovieSortTypeEnum.LATEST.getType())) {
                redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TOPIC_MOVIEID_LATEST, sid, fromId);
            }
            //时长
            else if (sortType.equalsIgnoreCase(KpnMovieSortTypeEnum.DURATION.getType())) {
                redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_TOPIC_MOVIEID_DURATION, sid, fromId);
            }
        ...
        //频道
            ...
            //最热 默认
            String redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_CHANNEL_MOVIEID_VV, sid, fromId);
            //最新
            if (sortType.equalsIgnoreCase(KpnMovieSortTypeEnum.LATEST.getType())) {
                redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_CHANNEL_MOVIEID_LATEST, sid, fromId);
            }
            //时长
            else if (sortType.equalsIgnoreCase(KpnMovieSortTypeEnum.DURATION.getType())) {
                redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_CHANNEL_MOVIEID_DURATION, sid, fromId);
            }
        ...
        //热门VIP推荐
            ...
            String redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_VIP_MOVIEID_VV, sid);
        ...
        //最新
            ...
            String redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_ALL_MOVIEID_LATEST, sid);
        ...
        //最热
            ...
            //最热 默认
            String redisKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_ALL_MOVIEID_VV, sid, fromId);
    ...
    public List<KpnSiteMovieBaseVo> searchSiteVipMovieTop5(Long sid) {
        String siteVipMovieIdsKey = StrUtil.format(MarksixConstants.RedisKey.KPN_SITE_VIP_MOVIEID_VV, sid);
        ...
        if (ObjectUtil.isEmpty(siteMovieVvObj)) {
            ...
            try {
                ...
                if (ObjectUtil.isEmpty(siteMovieVvObj)) {
                    ...
                    RedisRepository.setExpire(siteMovieVvKey, vv, MarksixConstants.RedisKey.EXPIRE_TIME_30_DAYS, TimeUnit.SECONDS);
                }
            } catch (Exception e) {
        ...
        if (ObjectUtil.isEmpty(siteMovieFavoritesObj)) {
            ...
            try {
                ...
                if (ObjectUtil.isEmpty(siteMovieFavoritesObj)) {
                    ...
                    RedisRepository.setExpire(siteMovieFavoritesKey, favorites, MarksixConstants.RedisKey.EXPIRE_TIME_30_DAYS, TimeUnit.SECONDS);
                }
            } catch (Exception e) {
                log.error(e.getMessage(), e);
            } finally {
