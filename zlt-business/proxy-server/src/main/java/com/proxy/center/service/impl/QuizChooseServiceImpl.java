...
import com.proxy.center.mapper.QuizChooseMapper;
import com.proxy.center.model.dto.QuizChooseDto;
import com.proxy.center.service.INumberAttributesService;
import com.proxy.center.service.IQuizChooseService;
...
public class QuizChooseServiceImpl extends SuperServiceImpl<QuizChooseMapper, QuizChoose> implements IQuizChooseService {
    ...
    @Override
    public Result saveOrUpdateQuizChoose(QuizChoose quizChoose, SysUser user) {
        if (null != quizChoose.getId() && 0 != quizChoose.getId()) {
            quizChoose.setUpdateTime(new Date());
            quizChoose.setUpdateBy(null != user ? user.getUsername() : quizChoose.getUpdateBy());
        } else {
            quizChoose.setCreateBy(null != user ? user.getUsername() : quizChoose.getCreateBy());
